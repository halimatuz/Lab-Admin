        ...
        <div class="col-12">
          <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                    <h6 class="text-primary">Assign Sampler STPS For <?= $sk_number->name_int; ?></h6>
                    <p class="font-weight-bold">No. <?= $sk_number->sk_sample; ?></p>
                    <hr>
                    <?php 
                    if($this->uri->segment(2) == 'update_sampler_stps') {
                    foreach ($special_assign as $sa) : ?>
                        <form action="<?= base_url('D_superadmin/update_sampler_stps_action') ?>" method="POST">
                            ...
                            <button type="submit" class="btn btn-primary">Update Sampler STPS</button>
                            <button type="reset" class="btn btn-danger">Reset</button>
                            <a href="<?php echo base_url('D_superadmin/add_sampler_stps/') . $sk_number->id_sk ?>" class="btn btn-danger">Cancel</a>
                        </form>
                    <?php endforeach; } else {?>
                        <form action="<?= base_url('D_superadmin/add_sampler_stps_action') ?>" method="POST">
                            <div class="form-group">
                                <label>Sampler</label>
                            ...
                            <button type="submit" class="btn btn-primary">Add Sampler STPS</button>
                            <button type="reset" class="btn btn-danger">Reset</button>
                        </form>
                    <?php } ?>
                    </div>
                            ...
                            <tbody>
                                ...
                                foreach ($assign_sampler as $row) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?= base_url('D_superadmin/update_sampler_stps/') . $row->id_assign . '/' . $row->id_sk ?>"class="btn btn-primary"><i class="fas fa-edit"></i></a>
                                            <a href="<?= base_url('D_superadmin/delete_sampler_stps/') . $row->id_assign . '/' . $row->id_sk ?>"class="btn btn-danger tombol-hapus"><i class="fas fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
